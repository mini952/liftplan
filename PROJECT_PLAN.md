...
**Phase 3 — Muscle heatmap (top priority). ◐ In progress (2026-07-12).**
Body diagram with the 9 general areas; switch between Area / Group granularity; adjustable involvement floor (R6). *Resolved this phase:* the diagram substrate — 9 areas, front + back schematic figure, one silhouette driving both grains via `data-group`/`data-area` (D-37, resolves P-02); original schematic artwork as a **swappable skin** (D-38); the 5 fully-deep groups as ghosted chips (D-39); the **shading metric** — intensity-weighted default, set-count **role-gated** to Primary+Secondary, count-once roll-ups, validated against the real fact table (D-40, resolves P-01); the **involvement floor** default (ignore intensity ≤2) with relative heat-normalization for MVP (D-41, resolves P-04); and the **UI palette** — aqua accent, warm ramp reserved for data (D-42, resolves P-19). **Specific-muscle granularity is deferred to Phase 8** (the group-grain seed makes a Specific view redundant until the re-analysis differentiates heads; it is an additive subdivision of this same figure — no substrate change; see D-37 and Phase 8). *Progress:* the shading engine is built and validated headlessly against the real embedded data (density + roll-up + floor assertions passing); integration into the Plan view (inline SVG, toggles, floor slider, deep-group chips, side-list, re-theme) is underway. *Deliverable:* the visual imbalance view during planning.
...
**Immediate next action (Phase 3 — Muscle heatmap, in progress):** the diagram substrate and all Phase-3 decisions are settled (D-37–D-42; P-01/P-02/P-04/P-19 resolved), and the shading engine is built and validated headlessly against the real embedded data. **Specific-muscle granularity is deferred to Phase 8** — Phase 3 ships **Area + Group** views. Remaining work: integrate into the Plan view — inline the group SVG (with the `MA0004` lumbar-tag fix), wire the engine, and add the Area/Group toggle, metric toggle, involvement-floor slider, deep-group chips, per-zone side-list, and the D-42 re-theme — then run a targeted Phase-3 validation checklist before wrap-up.
